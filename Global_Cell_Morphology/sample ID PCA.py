...
Inputs:
  ...
  Each Excel must include a 'SampleID' column + numeric feature columns.
...
def load_numeric_with_group(path: str, group_name: str) -> pd.DataFrame:
    ...
    if "SampleID" not in df.columns:
        raise ValueError(f"'SampleID' column not found in file:\n  {path}")
...
SAVE_PREFIX = "PCA_groupColor_sampleShape_gradient"

# Colors for cell type groups (paper style)
...
# Gradient strength per group (how much to lighten samples)
# Smaller range = subtler; larger range = more separation.
# Good default: 0.05..0.55
SHADE_MIN = 0.05
SHADE_MAX = 0.55
...
# ==============================
n_feat = set([c for c in normal.columns if c not in ["Group", "SampleID"]])
b_feat = set([c for c in beta.columns   if c not in ["Group", "SampleID"]])
r_feat = set([c for c in nras.columns   if c not in ["Group", "SampleID"]])
...
# Keep only selected features + Group + SampleID
normal = normal[features + ["Group", "SampleID"]].copy()
beta   = beta[features + ["Group", "SampleID"]].copy()
nras   = nras[features + ["Group", "SampleID"]].copy()

df = pd.concat([normal, beta, nras], ignore_index=True)

# Drop rows with NaNs
df = df.dropna(subset=features + ["Group", "SampleID"]).reset_index(drop=True)
...
#   2) SampleID (shape + shaded colors per group)
# ------------------------------
...
# Sample legend: show each sample with its marker, and (if present) use CTNNB1 shade as representative;
# if a sample exists in multiple groups, it will still show one entry (keeps legend compact).
sample_handles = []
for s in unique_samples:
    # pick a representative color for legend: first group where this sample exists
    rep_color = None
    for g in ["Normal", "CTNNB1", "Nras"]:
